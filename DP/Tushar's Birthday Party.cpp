...
int helper(const vector<int> &B, const vector<int> &C, int cap, int n, vector<vector<int>> &dp)
{
    if(cap==0)
    {
        return 0;
    ...
    if(B[n-1]<=cap)
    {
        return dp[cap][n] = min(C[n-1]+helper(B, C, cap-B[n-1], n, dp), helper(B, C, cap, n-1, dp));
    }
    else
    {
        return dp[cap][n] = helper(B, C, cap, n-1, dp);
    }
}
